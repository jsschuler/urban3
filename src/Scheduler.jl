...
function step!(state::ModelState)
    ...
    firm_reviews!(state)
    entrepreneur_phase!(state)
    commercial_developer_phase!(state)
    release_expired_leases!(state)
    ...
    developer_collect_rents!(state)
    developer_service_debt!(state)
    firm_contraction_expansion!(state)
    refresh_spatial_access!(state)
    worker_housing_search!(state)
    worker_exit!(state)
    developer_update!(state)
    immigration_phase!(state)
    refresh_spatial_access!(state)
    road_firm_phase!(state)
    land_developer_phase!(state)
    road_developer_phase!(state)
