...
def clear_scene():
    for obj in list(bpy.context.scene.objects):
        if obj.name.startswith("lot_") or obj.name.startswith("road_"):
...
def make_road(road, scale):
    ax = road["from_x"] * scale
    ay = road["from_y"] * scale
    bx = road["to_x"] * scale
    by = road["to_y"] * scale
    dx = bx - ax
    dy = by - ay
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        depth=length,
        location=(cx, cy, -0.05),
        rotation=(math.pi / 2, 0, angle_z),
    )
    obj = bpy.context.object
    obj.name = f"road_{road['id']}"
    obj.data.materials.append(material("road_gold", (0.82, 0.57, 0.11, 1.0)))
